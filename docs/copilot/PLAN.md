...
- [0. コードベース調査結果](#0-%E3%82%B3%E3%83%BC%E3%83%89%E3%83%99%E3%83%BC%E3%82%B9%E8%AA%BF%E6%9F%BB%E7%B5%90%E6%9E%9C)
- [1. リファクタリング作業計画](#1-%E3%83%AA%E3%83%95%E3%82%A1%E3%82%AF%E3%82%BF%E3%83%AA%E3%83%B3%E3%82%B0%E4%BD%9C%E6%A5%AD%E8%A8%88%E7%94%BB)
- [2. 詳細TODO](#2-%E8%A9%B3%E7%B4%B0todo)
- [3. marimoノートブックテスト戦略（将来）](#3-marimo%E3%83%8E%E3%83%BC%E3%83%88%E3%83%96%E3%83%83%E3%82%AF%E3%83%86%E3%82%B9%E3%83%88%E6%88%A6%E7%95%A5%E5%B0%86%E6%9D%A5)
...
1. ✅ `src/` ディレクトリを削除
1. ✅ `tests/` ディレクトリを削除
1. ✅ `pyproject.toml` からビルド設定・CLI設定を削除
1. ✅ `README.md` をnotebooks/中心の構成に更新
1. ✅ `docs/DESIGN.md` をシンプルな構成に更新
1. ✅ `.github/copilot-instructions.md` を更新
1. ✅ `.pre-commit-config.yaml` からmypy設定を削除
1. ✅ `.github/workflows/ci.yml` からtypecheck/test jobを削除
...
1. src/の定数・設定値のみをnotebooks/から参照
1. 将来的にロジックも段階的に移行
...
1. **早期発見**: コミット前に問題を検出
1. **リグレッション防止**: 既存機能が壊れないことを保証
1. **ドキュメント化**: テストがノートブックの期待動作を示す
1. **リファクタリング支援**: 安心してコードを改善できる
...
1. **リアクティビティ**: 変数変更時に依存セルが自動再実行
1. **表示ルール**: `return`または評価式で結果を表示
1. **サンドボックス**: インライン依存関係で隔離環境作成
...
1. **値の注入**: セル関数に直接引数を渡す
1. **モックオブジェクト**: `unittest.mock.Mock`で代替
...
1. **失敗時の対応**: テストが失敗したらどうするか？

   - コミットブロック？
   - 警告のみ？
   - マージブロック？

1. **Phase 4の必要性**: 統合テストは本当に必要か？
